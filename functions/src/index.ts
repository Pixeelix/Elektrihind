...
const regions = ["EE", "LV", "LT", "FI"] as const;

type Region = (typeof regions)[number];
type NotificationDirection = "max" | "min";
const chartResolutions = ["15min", "1h"] as const;
type ChartResolution = (typeof chartResolutions)[number];
...
interface DeviceDocument extends DeviceRegistration {
  lastMaxNotifiedDate?: string;
  lastMinNotifiedDate?: string;
}

interface ActiveDevice extends DeviceDocument {
  id: string;
...
export const syncNotificationDevice = onCall({ region: functionsRegion }, async (request) => {
  ...
  await db.collection(devicesCollection).doc(request.auth.uid).set(
    removeUndefined({
      ...registration,
...
async function runPriceCheck(): Promise<PriceCheckSummary> {
  const window = tomorrowWindow();
  const prices = await fetchTomorrowPrices(window);
  const incomplete = regions.filter(
    (region) => prices[region].length !== window.expectedIntervalCount,
  );
  ...
  for (const region of regions) {
    const pointsByResolution: Record<ChartResolution, PricePoint[]> = {
      "15min": prices[region],
...
async function fetchTomorrowPrices(
  window: ReturnType<typeof tomorrowWindow>,
): Promise<Record<Region, PricePoint[]>> {
...
async function activeDevices(region: Region): Promise<ActiveDevice[]> {
  ...
  return snapshot.docs
    .map((doc) => ({ id: doc.id, ref: doc.ref, ...(doc.data() as DeviceDocument) }))
...
function aggregateToHourly(points: PricePoint[]): PricePoint[] {
  const buckets = new Map<number, { sum: number; count: number }>();

  for (const point of points) {
    const hourStart = Math.floor(
      DateTime.fromSeconds(point.timestamp, { zone: "utc" })
...
function chartResolution(device: DeviceDocument): ChartResolution {
...
function displayPrice(rawMWh: number, device: DeviceDocument): number {
  const taxRate = device.includeTax ? taxRates[device.region] : 1;
  return (rawMWh * taxRate) / divider(device.unit);
}

function formatPrice(rawMWh: number, device: DeviceDocument): string {
...
function notificationTitle(direction: NotificationDirection, language: string): string {
...
function formatHour(timestampSeconds: number, region: Region): string {
...
function parseDeviceRegistration(data: unknown): DeviceRegistration {
  ...
  try {
    ...
    const registration: DeviceRegistration = {
      ...
      platform: oneOf(stringField(input, "platform"), ["ios"] as const, "platform"),
      ...
      chartResolution: oneOf(
        optionalString(input, "chartResolution") ?? "1h",
        chartResolutions,
        "chartResolution",
      ),
      notifyMaxEnabled,
    ...
    if (appVersion) registration.appVersion = appVersion;
    if (buildNumber) registration.buildNumber = buildNumber;
