//
//  ElektrihindWidgetExtension.swift
//  ElektrihindWidgetExtension
//
...
// MARK: - Timeline Entry
struct ElektrihindEntry: TimelineEntry {
    let date: Date
...
// MARK: - Provider
struct ElektrihindProvider: TimelineProvider {
    func placeholder(in context: Context) -> ElektrihindEntry {
        ElektrihindEntry(date: Date(), priceText: "--", unit: "€/kWh", timeText: "--:--", regionCode: "EE", prices: [])
    }

    func getSnapshot(in context: Context, completion: @escaping (ElektrihindEntry) -> Void) {
        buildEntry { entry in
            completion(entry)
        }
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<ElektrihindEntry>) -> Void) {
    ...
    private func buildEntry(completion: @escaping (ElektrihindEntry) -> Void) {
        let region = WidgetSettings.region()
        let unit = WidgetSettings.unit()
        let includeTax = WidgetSettings.includeTax()
        let taxRate = WidgetSettings.taxRate(for: region)
...
// MARK: - Widget View
struct ElektrihindWidgetEntryView: View {
    var entry: ElektrihindProvider.Entry
    ...
    var body: some View {
        ...
        if #available(iOSApplicationExtension 17.0, *) {
            content
                ...
                .widgetURL(URL(string: "elektrihind://today"))
        } else {
            content
...
// MARK: - Previews
struct ElektrihindWidget_Previews: PreviewProvider {
    ...
    static var previews: some View {
        Group {
            ElektrihindWidgetEntryView(
                entry: ElektrihindEntry(date: Date(), priceText: "0,1234", unit: "€/kWh", timeText: "12:00", regionCode: "EE", prices: samplePrices)
            )
            .previewContext(WidgetPreviewContext(family: .systemSmall))

            ElektrihindWidgetEntryView(
                entry: ElektrihindEntry(date: Date(), priceText: "98,7", unit: "€/MWh", timeText: "08:15", regionCode: "FI", prices: samplePrices)
            )
